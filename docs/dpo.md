### 如何使用 pytorch 实现 dpo 训练

本文参考[链接](https://github.com/jingyaogong/minimind)实现了一个混合精度的 dpo 训练流程。

特点：

- 每条数据包含一个 chosen 和 rejected，代表人类选择的倾向
- 训练过程中包含了一个参考模型，该参考模型用来给出指导性的 token 序列概率分布
- 损失为训练模型与参考模型各自"接受-拒绝概率差"的差，"接受-拒绝概率差"为 chosen 平均概率与 reject 平均概率的差
